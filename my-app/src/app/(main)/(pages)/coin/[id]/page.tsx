...
import { fetchCoinData, fetchHistoricalData } from '@/app/api/coinData/route';
...
const Coin = () => {
  ...
  }, [Coinid, Coin, RecentlyViewed, setRecentlyViewed]);

  if (coinData && HistoricalChartData) {
    return (
      <div className='w-full h-[1500px]  flex '>
        <CommonHeader displayCoin={displayCoin} setDisplayCoin={setDisplayCoin} />
        <div className='individual-Coin content flex w-full h-full gap-5 min-h-[100vh]'>
          <div className='left'>
            <div className='flex gap-2'>
              <h1 className='text-2xl font-bold'>{Coinid.toLocaleUpperCase()}</h1>
              <Image
               //@ts-ignore
              ...
              <div className='Performace flex flex-col gap-5 p-5'>
                ...
                <div className='flex gap-5'>
                  ...
                  <Progress
                    ...
                    {((coinData?.market_data.current_price["usd"] - coinData?.market_data.low_24h["usd"]) / (coinData?.market_data.high_24h["usd"] - coinData?.market_data.low_24h["usd"])) * 100}
                    className="w-[60%]" 
                  />
                  <div className="flex flex-col gap-1">
                    <p>Today's High</p>
                     {/*@ts-ignore */}
                    <p className='text-sm text-green-600'>{coinData?.market_data.high_24h["usd"]}</p>
                  </div>
                </div>
              </div>
